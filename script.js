...
async function copy(text) {
  try { await navigator.clipboard.writeText(text); alert("Copié ✔"); }
    ...
    document.execCommand('copy'); document.body.removeChild(t); alert("Copié ✔");
  }
}
...
function linkOrText(label, value, href){
  ...
  li.appendChild(span);
  if (href) {
    const a = document.createElement('a'); a.href = href; a.textContent = value; a.target = "_blank"; a.rel="noopener";
    li.appendChild(a);
  } else {
...
function renderProfile(profileId){
  ...
  Object.keys(groups).forEach(title => {
    ...
    (groups[title] || []).forEach(skill => {
      const span = document.createElement('span'); span.className="chip"; span.textContent = skill;
  ...
  if (c.location) v.push(`ADR;TYPE=HOME:;;${c.location};;;;`);
  v.push("END:VCARD");
